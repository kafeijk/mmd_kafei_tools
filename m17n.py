...
translation_dict = {
    ...
    "en_US": {
        ...
        ("*", "设置三点照明"): "Set three-point lighting",
        ...
        ("*", "移除丢失外部数据"): "Remove Missing External Data",
        ...
        ("*", "渲染预览图\n渲染逻辑相当于点击预览按钮并执行图像渲染\n勾选批量参数后，按钮将切换为批量渲染模式"): "Render the preview image\nThe rendering logic is equivalent to clicking the preview button and performing an image render\nWhen batch parameters are enabled, the button switches to batch rendering mode",
        ...
        ("*", "根据当前设置的参数生成预览相机并切换到摄像机视角，重复点击时会更新预览相机参数"): "Generate a preview camera based on the current settings and switch to its view. Clicking again updates the preview camera parameters.",
        # Batch parameter
        ("*", "批量"): "Batch",
